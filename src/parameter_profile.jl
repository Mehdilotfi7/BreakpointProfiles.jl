...
`best_loss + chi2_threshold(1, 0.95)`; for L1 / Laplace likelihoods use
`best_loss + bootstrap_threshold(...)`.
"""
...
# ============================================================================
# Bootstrap threshold for non-χ² losses (e.g. L1 / Laplace)
# ============================================================================

function _build_loss_free_fn(objective::Function, fixed_val::Float64, idx::Int, free::Vector{Int}, n::Int)
    function loss_free(x_free)
        θ = Vector{Float64}(undef, n)
        θ[idx] = fixed_val
        k = 1
...
"""
    bootstrap_threshold(data_generator, objective, best_params, idx;
                        n_boot=200, alpha=0.95, lb, ub, optimizer)

Compute an empirical `alpha`-quantile profile-likelihood threshold for parameter
`idx` by a non-parametric bootstrap.

`data_generator()` must return a bootstrap dataset. `objective(params, data)` must
return the scalar loss for a parameter vector `params` and dataset `data`. The
returned value is a **loss margin** (i.e. `threshold - best_loss`). To use it
with `profile_parameter`, pass `best_loss + margin` as the absolute `threshold`.

For each bootstrap replicate:
1. Refit the model to the bootstrap data to obtain the bootstrap MLE `θ̂*`.
2. Fix the profiled parameter `idx` to the **original MLE value**
   `best_params[idx]` (not the bootstrap MLE).
3. Re-optimise the remaining parameters on the bootstrap data subject to that
   constraint.
4. Record the profile-likelihood ratio statistic
   `L(θ_constrained | data*) - L(θ̂* | data*)`.

Fixing the parameter at the original MLE value is essential: if the parameter
were fixed at its bootstrap MLE, the constrained optimum would equal the
unconstrained bootstrap optimum by construction and the statistic would be zero.
The empirical `alpha`-quantile of the recorded statistics is returned as the
threshold margin.

This is the appropriate threshold for losses where Wilks' theorem does not apply
(e.g. L1 / Laplace likelihoods). For Gaussian (L2) likelihoods, use
`chi2_threshold` instead.
"""
function bootstrap_threshold(data_generator::Function, objective::Function, best_params::Vector{Float64},
                               idx::Int; n_boot::Int=200, alpha::Float64=0.95,
                               lb::Vector{Float64}=fill(-Inf, length(best_params)),
                               ub::Vector{Float64}=fill(Inf, length(best_params)),
                               optimizer::AbstractPLEOptimizer=default_multistart_bobyqa_config())
    n = length(best_params)
    free = setdiff(1:n, idx)
    lb_free = lb[free]
    ub_free = ub[free]
    fixed_val = best_params[idx]
    stats = Float64[]
    ...
    for b in 1:n_boot
        data_b = try
            ...
            @warn "Bootstrap data generation failed: $e"
            continue
        end
        obj_b = x -> objective(x, data_b)
        loss_boot, params_boot = try
            optimize_ple(obj_b, best_params, lb, ub, optimizer)
        catch e
            @warn "Bootstrap refit failed: $e"
            continue
        end
        loss_free_fn = _build_loss_free_fn(obj_b, fixed_val, idx, free, n)
        loss_profile, _ = try
            optimize_ple(loss_free_fn, params_boot[free], lb_free, ub_free, optimizer)
        catch e
            @warn "Bootstrap profile failed: $e"
            continue
        end
        stat = loss_profile - loss_boot
        if isfinite(stat) && stat >= 0
            push!(stats, stat)
        end
    end

    isempty(stats) && error("All bootstrap replicates failed; cannot compute threshold")
    return quantile(stats, alpha)
end

"""
    bootstrap_thresholds(data_generator, objective, best_params;
                         n_boot=200, alpha=0.95, lb, ub, optimizer,
                         indices=1:length(best_params))

Compute a separate profile-likelihood bootstrap threshold for each parameter in
`indices`. Returns a vector `thresholds` where `thresholds[j]` is the margin to
add to `best_loss` for parameter `indices[j]`.

This is the recommended way to set thresholds for L1 / Laplace losses where a
single global χ² threshold is not appropriate. Each parameter has its own
identifiability, so each parameter gets its own threshold.

The computation is parallelised over bootstrap replicates using
`Threads.@threads`.
"""
function _bootstrap_threshold_replicate!(data_generator::Function, objective::Function,
                                          best_params::Vector{Float64},
                                          lb::Vector{Float64}, ub::Vector{Float64},
                                          optimizer::AbstractPLEOptimizer,
                                          indices::Vector{Int}, n::Int,
                                          all_stats::Vector{Vector{Float64}},
                                          n_failed::Threads.Atomic{Int})
    data_b = try
        data_generator()
    catch e
        @warn "Bootstrap data generation failed: $e"
    ...
    for (k, idx) in enumerate(indices)
        ...
        loss_profile, _ = try
            ...
            @warn "Bootstrap profile failed for parameter $idx: $e"
            continue
        end
        stat = loss_profile - loss_boot
        if isfinite(stat) && stat >= 0
            push!(all_stats[k], stat)
        end
    end
end

function bootstrap_thresholds(data_generator::Function, objective::Function, best_params::Vector{Float64};
                                n_boot::Int=200, alpha::Float64=0.95,
                                lb::Vector{Float64}=fill(-Inf, length(best_params)),
                                ub::Vector{Float64}=fill(Inf, length(best_params)),
                                optimizer::AbstractPLEOptimizer=default_multistart_bobyqa_config(),
                                indices::Vector{Int}=collect(1:length(best_params)))
    n = length(best_params)
    n_idx = length(indices)
    all_stats = [Float64[] for _ in 1:n_idx]
    n_failed = Threads.Atomic{Int}(0)

    # Avoid nested Threads.@threads when the inner optimizer already uses threads.
    threaded_outer = !uses_internal_threads(optimizer)
    if threaded_outer
        Threads.@threads for b in 1:n_boot
            _bootstrap_threshold_replicate!(data_generator, objective, best_params, lb, ub,
                                            optimizer, indices, n, all_stats, n_failed)
        end
    else
        for b in 1:n_boot
            _bootstrap_threshold_replicate!(data_generator, objective, best_params, lb, ub,
    ...
    for (k, idx) in enumerate(indices)
        stats = all_stats[k]
        if isempty(stats)
            @warn "All bootstrap replicates failed for parameter $idx; using 0.0 threshold margin"
            push!(thresholds, 0.0)
        else
            push!(thresholds, quantile(stats, alpha))
        end
    end
